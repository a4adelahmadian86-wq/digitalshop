...
class ApiStorageProvider implements StorageProviderInterface
{
    ...
    protected function client()
    {
        ...
        $name = $this->config['header_name'] ?? 'Authorization';
        $prefix = $this->config['header_prefix'] ?? 'Bearer ';
        if ($this->apiKey !== '') {
            $headers[$name] = $prefix.$this->apiKey;
        }
    ...
    public function put(UploadedFile $file, string $path): string
    {
        ...
        $response = $request->post($this->url('upload_path'), [
            ($this->config['path_field'] ?? 'path') => $path,
        ]);
        if ($response->failed()) {
    ...
    {
        $r = $this->client()->delete($this->url('delete_path'), [
            ($this->config['path_field'] ?? 'path') => $path,
        ]);
    ...
    {
        $r = $this->client()->get($this->url('exists_path'), [
            ($this->config['path_field'] ?? 'path') => $path,
        ]);
        if ($r->status() === 404 || $r->failed()) {
            return false;
        }

        return (bool) (data_get($r->json(), $this->config['exists_response'] ?? 'exists') ?? true);
    }

    public function download(string $path, ?string $name = null)
    {
        $r = $this->client()->get($this->url('download_path'), [
            ($this->config['path_field'] ?? 'path') => $path,
        ]);
        if ($r->failed()) {
            abort($r->status());
        ...
        return response($r->body(), Response::HTTP_OK, [
    ...
    {
        $path = trim((string) ($this->config['test_path'] ?? ''));
        $r = $this->client()->get($path !== '' ? $this->url('test_path') : $this->endpoint);
    ...
    public function usage(): array
    {
        ...
        try {
            $r = $this->client()->get($this->url('usage_path'));
            if ($r->successful()) {
                $j = $r->json();

                return [
                    'bytes' => (int) (data_get($j, $this->config['usage_bytes_response'] ?? 'used_bytes') ?? data_get($j, 'data.used_bytes') ?? 0),
                    'files' => (int) (data_get($j, $this->config['usage_files_response'] ?? 'used_files') ?? data_get($j, 'data.used_files') ?? 0),
                ];
            }
        } catch (\Throwable $e) {
            report($e);
    ...
    {
        $path = trim((string) ($this->config['cleanup_path'] ?? ''));
        if ($path === '') {
            return 0;
        }
        try {
            $r = $this->client()->post($this->url('cleanup_path'), [
                'older_than_seconds' => max(1, $seconds),
            ]);
            if ($r->successful()) {
                return (int) (data_get($r->json(), 'deleted') ?? data_get($r->json(), 'data.deleted') ?? 0);
            }
        } catch (\Throwable $e) {
            report($e);
        }
