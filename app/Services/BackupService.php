...
class BackupService
{
    ...
    public function run(): array
    {
        ...
        foreach (['internal','external'] as $location) {
            $provider = $this->storage->select('backups',$location,null,filesize($file));
            if (!$provider) continue;
            try {
                $upload = $this->storage->upload($provider,new UploadedFile($file,$name,'application/sql',null,true),'backups/'.now()->format('Y/m').'/'.$name);
                $uploaded[] = ['location'=>$location,'provider_id'=>$provider->id,'path'=>$upload];
            } catch (\Throwable $e) {
                report($e);
            }
        }
