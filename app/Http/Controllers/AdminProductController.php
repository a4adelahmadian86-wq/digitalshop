...
class AdminProductController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $product = DB::transaction(function () use ($data,$uploads,$provider,$slug) {
            $product = Product::create([
                ...
                'title'=>$data['title'],'slug'=>$slug,'short_description'=>$data['short_description']??null,
    ...
    public function update(Request $request,Product $product)
    {
        ...
        DB::transaction(function()use($data,$product,$provider){
            $product->update(['category_id'=>$data['category_id'],'storage_provider_id'=>$provider->id,'title'=>$data['title'],'slug'=>$data['slug'] ?: Str::slug($data['title']) ?: 'product-'.Str::lower(Str::random(10)),'short_description'=>$data['short_description']??null,'description'=>$data['description']??null,'price'=>$data['price'],'is_published'=>false]);
    ...
    private function validateData(Request $request,?Product $product,bool $creating):array
    {
        ...
        return $request->validate([
            'category_id'=>['required','exists:categories,id'],'storage_provider_id'=>['required','exists:storage_providers,id'],
