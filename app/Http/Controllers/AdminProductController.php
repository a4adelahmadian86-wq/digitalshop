...
class AdminProductController extends Controller
{
    ...
    public function store(Request $request, ProductKnowledgeService $knowledge)
    {
        ...
        $provider = $this->resolveProvider($data['storage_provider_id'] ?? null);
        $slug = $data['slug'] ?: Str::slug($data['title']);
        if ($slug === '') $slug = 'product-' . Str::lower(Str::random(10));

        $product = DB::transaction(function () use ($data, $uploads, $provider, $slug) {
            $product = Product::create([
                'category_id' => $data['category_id'], 'storage_provider_id' => $provider->id,
                'title' => $data['title'], 'slug' => $slug,
                'short_description' => $data['short_description'] ?? null,
                'description' => $data['description'] ?? null, 'price' => $data['price'], 'is_published' => false,
            ]);
        ...
        return redirect()->route('admin.products.index')->with('success', 'محصول ثبت شد و فایل آن برای پایگاه دانش آماده‌سازی شد.');
    }
    ...
    public function update(Request $request, Product $product, ProductKnowledgeService $knowledge)
    {
        ...
        return redirect()->route('admin.products.index')->with('success', 'محصول ویرایش شد و پایگاه دانش آن به‌روزرسانی شد.');
    }
    ...
    private function resolveProvider(?int $id): StorageProvider
    {
        $provider = $id ? StorageProvider::find($id) : StorageProvider::where('is_active', true)->where('is_default', true)->first();
        if (!$provider) $provider = StorageProvider::where('is_active', true)->orderBy('id')->first();
        abort_unless($provider && $provider->is_active, 422, 'هیچ Storage Provider فعالی برای محصول وجود ندارد.');
        return $provider;
    }
    ...
    private function attachUploads(Product $product, $uploads): void
    {
        ...
        foreach ($uploads->values() as $i => $upload) {
            ProductFile::create([
                'product_id' => $product->id, 'storage_provider_id' => $upload->storage_provider_id,
                'original_name' => $upload->original_name, 'stored_name' => basename($upload->stored_path),
                'storage_path' => $upload->stored_path, 'mime_type' => $upload->mime_type,
                'extension' => $upload->extension, 'size' => $upload->size, 'sha256' => $upload->sha256,
                'sort_order' => $start + $i,
            ]);
    ...
    private function validateData(Request $request, ?Product $product): array
    {
        ...
        return $request->validate([
            'category_id' => ['required', 'exists:categories,id'], 'storage_provider_id' => ['nullable', 'integer', 'exists:storage_providers,id'],
            'title' => ['required', 'string', 'max:255'], 'slug' => ['nullable', 'string', 'max:255', 'regex:/^[a-z0-9-]+$/', $unique],
            'short_description' => ['nullable', 'string', 'max:1000'], 'description' => ['nullable', 'string', 'max:100000'],
            'price' => ['required', 'integer', 'min:0'], 'upload_ids' => ['nullable', 'array', 'max:30'], 'upload_ids.*' => ['integer'],
        ]);
    }
}
