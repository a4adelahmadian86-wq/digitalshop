...
class AdminProductController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        DB::transaction(function () use ($data, $uploads, $provider, $slug) {
    ...
    public function destroy(Product $product, \App\Services\Storage\StorageManager $storageManager)
    {
        foreach ($product->files as $file) {
            try { $storageManager->provider($file->storageProvider)->delete($file->storage_path); } catch (\Throwable $e) { report($e); }
        }
        if ($product->storage_path && $product->storageProvider) {
            try { $storageManager->provider($product->storageProvider)->delete($product->storage_path); } catch (\Throwable $e) { report($e); }
        }
        $product->delete();
    ...
    private function validateData(Request $request, ?Product $product): array
    {
        ...
        return $request->validate([
            ...
            'slug' => ['nullable', 'string', 'max:255', 'regex:/^[a-z0-9-]+$/', $unique],
            'short_description' => ['nullable', 'string', 'max:1000'],
            'description' => ['nullable', 'string', 'max:100000'],
            'price' => ['required', 'integer', 'min:0'],
            'upload_ids' => ['nullable', 'array', 'max:30'],
            'upload_ids.*' => ['integer'],
        ]);
    }
}
