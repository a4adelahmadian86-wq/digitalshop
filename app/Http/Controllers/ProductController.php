...
class ProductController
{
    public function show(Product $product, CustomerIntentService $intent, ProductSearchService $searchService)
    {
        ...
        session(['recent_products' => array_slice($recent, 0, 8)]);

        $downloadItem = auth()->check()
            ? OrderItem::where('product_id', $product->id)
                ->whereHas('order', fn ($q) => $q->where('user_id', auth()->id())->whereIn('status', ['paid', 'completed']))
                ->first()
            : null;
        ...
        $pdfPreview = $pdfFile
            ? ProductPreview::where('product_file_id', $pdfFile->id)
                ->where('source_sha256', $pdfFile->sha256)
                ->latest('id')
                ->first()
            : null;
        ...
        return view('product', compact('product', 'downloadItem', 'related', 'recommended', 'pdfFile', 'pdfPreview', 'previewExcerpt'));
    }
    ...
    public function search(Request $request, CustomerIntentService $intent, ProductSearchService $searchService)
    {
        ...
        if ($q !== '') {
            $intent->record('search', $q);
            $recent = session('recent_searches', []);
            $recent = array_values(array_filter(array_diff($recent, [$q])));
            array_unshift($recent, $q);
            session(['recent_searches' => array_slice($recent, 0, 8)]);
        }

        $base = Product::where('is_published', true)->when($categoryId > 0, fn ($x) => $x->where('category_id', $categoryId));
        if ($q === '') {
            $products = $base->latest('id')->paginate(12)->withQueryString();
        } else {
            $all = $searchService->search($q, 48);
            if ($categoryId > 0) $all = $all->filter(fn ($p) => (int) $p->category_id === $categoryId)->values();
            $page = max(1, (int) $request->input('page', 1));
            $perPage = 12;
            $slice = $all->slice(($page - 1) * $perPage, $perPage)->values();
            $products = new LengthAwarePaginator($slice, $all->count(), $perPage, $page, ['path' => $request->url(), 'query' => $request->query()]);
        }
