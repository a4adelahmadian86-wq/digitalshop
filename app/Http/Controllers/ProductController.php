...
class ProductController
{
    public function show(Product $product, CustomerIntentService $intent, ProductSearchService $searchService)
    {
        ...
        $related=Product::where('is_published',true)->where('id','<>',$product->id)->where(function($q)use($product){$q->where('category_id',$product->category_id);if($product->title)$q->orWhere('title','like','%'.Str::of($product->title)->words(4,'').'%');})->latest('id')->take(8)->get();
