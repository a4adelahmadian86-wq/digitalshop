...
class AppServiceProvider extends ServiceProvider
{
    public function register(): void
    {
        ...
        $this->app->singleton(AIProviderInterface::class, function () {
            $s = app(AiSettingsStore::class);
            $primary = $s->get('key', config('ai.key')) ? new GeminiProvider() : new NullAIProvider();
            $fallback = $s->get('fallback_key', config('ai.fallback_key')) ? new GapGptProvider() : new NullAIProvider();
    ...
    public function boot(): void
    {
        ...
        Route::middleware('web')->get('/Images/{path}', [\App\Http\Controllers\MediaController::class, 'legacy'])->where('path', '.*')->name('images.legacy');
    }
}
