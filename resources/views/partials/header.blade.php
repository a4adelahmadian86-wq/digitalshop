...
$navCategories = \App\Models\Category::query()->when(
    \Illuminate\Support\Facades\Schema::hasColumn('categories', 'is_active'),
    fn ($q) => $q->where('is_active', 1)
)->when(
    \Illuminate\Support\Facades\Schema::hasColumn('categories', 'sort_order'),
        ...
        <div class="container header-inner">
                ...
                <div class="nav-dropdown">
                    ...
                    <div class="category-mega">
                        ...
                        <div class="mega-grid">
                            ...
                            @empty
                                <a href="{{ route('products.index') }}" class="mega-item"><span class="mega-icon">◈</span><span><strong>همه فایل‌ها</strong><small>مشاهده فروشگاه</small></span></a>
                            @endforelse
                        </div>
                    </div>
                </div>
            ...
            <div class="header-actions">
                ...
                @auth
                    ...
                    <form method="POST" action="{{ route('logout') }}" style="margin:0">@csrf<button class="account-trigger" type="submit" aria-label="خروج"><span class="header-action-icon">↪</span><span class="header-action-label">خروج</span></button></form>
                @else
...
<aside id="cart-panel" class="cart-panel" aria-label="سبد خرید">
    ...
    <div class="cart-items">
        @forelse($cartProducts as $product)
            <div class="mini-cart"><div class="mini-cart-thumb">📁</div><div class="mini-cart-info"><span>{{ $product->title }}</span><strong>{{ number_format($product->price * ($cart[$product->id] ?? 1)) }} تومان</strong></div></div>
        @empty
...
@push('styles')
<style>.header-search>span svg,.header-action-icon svg{width:18px;height:18px;display:block}.header-actions form{margin:0}.header-actions .notification-center{display:flex}</style>
@endpush
