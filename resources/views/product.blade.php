...
<div class="container product-page-v2">
    <section class="product-hero">
        <div class="product-cover">
            <img src="{{ $product->thumbnail_url }}" alt="{{ $product->title }}" loading="eager" width="900" height="560">
            <small>شناسه محصول #{{ $product->id }} · {{ $product->files->count() }} فایل</small>
        </div>
        <div class="product-info">
            ...
            @endif
            <div class="product-price-wrap" id="productPrice">
                <span>قیمت محصول</span>
                <div class="product-price">{{ number_format($product->price) }} <small>تومان</small></div>
            </div>
            <div class="product-actions">
                ...
                @else
                    <form method="POST" action="{{ route('cart.add', $product) }}">
                        @csrf
                        <button class="product-action product-buy" type="submit"><x-icon name="cart" size="17"/> افزودن به سبد</button>
                    </form>
                @endif
                <a class="product-support" href="{{ auth()->check() ? route('support.create', ['related_type'=>'product','related_id'=>$product->id,'subject'=>'سؤال درباره محصول #'.$product->id]) : route('login') }}"><x-icon name="support" size="16"/> سؤال یا مشکل دارید؟</a>
            </div>
        </div>
    </section>
    ...
    <section class="product-description">
        <h2>توضیحات محصول</h2>
        <div class="description-body">{!! $product->description ?: '<p>توضیحات تکمیلی این محصول هنوز ثبت نشده است.</p>' !!}</div>
    </section>
    ...
    <section class="feedback-grid">
        <div class="feedback-card">
            ...
            @forelse($product->reviews as $review)
                <article class="review"><div class="review-top"><b>{{ $review->user?->first_name ?: 'خریدار' }}</b><span>{{ str_repeat('★', $review->rating) }}{{ str_repeat('☆', 5 - $review->rating) }}</span></div><p>{{ $review->body }}</p></article>
            @empty
                <p class="empty">هنوز نظری ثبت نشده است.</p>
            @endforelse
            @if($downloadItem)
                <form method="POST" action="{{ route('product.review.store', $product) }}" class="feedback-form">
                    @csrf
                    <h3>نظر شما</h3>
                    <select name="rating" required><option value="">امتیاز</option><option value="5">۵ — عالی</option><option value="4">۴ — خوب</option><option value="3">۳ — متوسط</option><option value="2">۲ — ضعیف</option><option value="1">۱ — بسیار ضعیف</option></select>
                    <textarea name="body" rows="4" maxlength="3000" placeholder="تجربه واقعی خود از فایل را بنویسید..." required></textarea>
                    <button class="product-action product-buy" type="submit">ثبت نظر</button>
                </form>
            @endif
        </div>
        ...
        <div class="feedback-card">
            ...
            @forelse($product->questions as $question)
                <article class="question"><div class="review-top"><b>{{ $question->user?->first_name ?: 'کاربر' }}</b></div><p>{{ $question->body }}</p>@foreach($question->answers as $answer)<div class="answer"><b>پاسخ</b><p>{{ $answer->body }}</p></div>@endforeach</article>
            @empty
                <p class="empty">هنوز پرسشی ثبت نشده است.</p>
            @endforelse
            @auth
                <form method="POST" action="{{ route('product.question.store', $product) }}" class="feedback-form">@csrf<h3>سؤال خود را بپرسید</h3><textarea name="body" rows="4" maxlength="3000" placeholder="چه چیزی درباره این محصول می‌خواهید بدانید؟" required></textarea><button class="product-action product-buy" type="submit">ثبت سؤال</button></form>
            @else
                <p class="login-hint">برای ثبت سؤال ابتدا وارد حساب کاربری شوید.</p>
            @endauth
        </div>
    </section>

    @if($related->count())
        <section class="recommend-section"><div class="section-head"><h2>محصولات مرتبط</h2><span>بر اساس دسته‌بندی</span></div><div class="product-grid">@foreach($related as $p)<a href="{{ route('product.show', $p) }}" class="mini-product"><div class="mini-cover"><img src="{{ $p->thumbnail_url }}" alt="{{ $p->title }}" loading="lazy"></div><b>{{ $p->title }}</b><small>{{ number_format($p->price) }} تومان</small></a>@endforeach</div></section>
    @endif
    @if($recommended->count())
        <section class="recommend-section"><div class="section-head"><h2>پیشنهاد برای شما</h2><span>بر اساس فعالیت ثبت‌شده</span></div><div class="product-grid">@foreach($recommended as $p)<a href="{{ route('product.show', $p) }}" class="mini-product"><div class="mini-cover"><img src="{{ $p->thumbnail_url }}" alt="{{ $p->title }}" loading="lazy"></div><b>{{ $p->title }}</b><small>{{ number_format($p->price) }} تومان</small></a>@endforeach</div></section>
    @endif
</div>
@endsection
