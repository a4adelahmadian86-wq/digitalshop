...
@section('content')
<div class="admin-page narrow"><div class="admin-page-head"><div><div class="admin-eyebrow">مدیریت دسته‌بندی‌ها</div><h1>ویرایش دسته‌بندی</h1><p>{{ $category->name }} · شناسه #{{ $category->id }}</p></div></div>
...
<div class="form-group"><label>نامک انگلیسی</label><input type="text" name="slug" value="{{ old('slug',$category->slug) }}" dir="ltr" required><small>برای لینک و سئو؛ در صورت نیاز مدیر می‌تواند آن را ویرایش کند.</small></div>
