@php
$path=request()->path();
$unread=(int)($panelUnread ?? $unreadNotifications ?? auth()->user()->unreadNotifications()->count());
@endphp
<nav class="panel-nav">
<div class="panel-nav-title">حساب کاربری</div>
<a class="{{ request()->routeIs('account.dashboard')?'active':'' }}" href="{{ route('account.dashboard') }}"><x-icon name="home" size="17"/>پیشخوان</a>
<a class="{{ request()->routeIs('account.profile','account.security')?'active':'' }}" href="{{ route('account.profile') }}"><x-icon name="user" size="17"/>پروفایل کاربری</a>
<a class="{{ request()->routeIs('account.orders*')?'active':'' }}" href="{{ route('account.orders') }}"><x-icon name="bag" size="17"/>سفارش‌ها</a>
<a class="{{ request()->routeIs('account.files')?'active':'' }}" href="{{ route('account.files') }}"><x-icon name="download" size="17"/>فایل‌های من</a>
<a class="{{ request()->routeIs('account.reader*')?'active':'' }}" href="{{ route('account.reader') }}"><x-icon name="eye" size="17"/>خوانشگر و اشتراک</a>
<a class="{{ request()->routeIs('account.club')?'active':'' }}" href="{{ route('account.club') }}"><x-icon name="gift" size="17"/>باشگاه مشتریان</a>
<a class="{{ request()->routeIs('account.wallet')?'active':'' }}" href="{{ route('account.wallet') }}"><x-icon name="wallet" size="17"/>کیف پول</a>
<a class="{{ request()->routeIs('account.notifications*')?'active':'' }}" href="{{ route('account.notifications') }}"><x-icon name="notification" size="17"/>اعلان‌ها @if($unread>0)<b class="nav-badge">{{ $unread }}</b>@endif</a>
<a class="{{ str_starts_with($path,'support')?'active':'' }}" href="{{ route('support.index') }}"><x-icon name="support" size="17"/>پشتیبانی و تیکت‌ها</a>
...
<div class="panel-nav-title">مدیریت سامانه</div>
@php
$groups=[
...
'workflow'=>['گردش کار','workflow.manage',['گردش کار','تأییدها','صف‌های کاری']],
'tasks'=>['وظایف','tasks.manage',['وظایف من','وظایف تیم','یادآوری‌ها']],
'security'=>['امنیت','security.manage',['مرکز امنیت','ورودها','نشست‌ها','رخدادهای امنیتی','گزارش حسابرسی']],
'integrations'=>['یکپارچه‌سازی‌ها','integrations.manage',['سرویس‌ها','API','کلیدهای API','وب‌هوک‌ها','سرویس‌های خارجی']],
...
'developer'=>['توسعه‌دهنده','developer.manage',['مستندات API','برنامه‌ها','OAuth','وب‌هوک‌ها','محیط آزمایشی']],
...
$sectionKeys=[
'users'=>['all','active','pending','roles','groups','activity'],'products'=>['all','create','draft','pending','approved','rejected','categories','features','pricing','reviews'],'orders'=>['all','pending','paid','failed','cancelled','completed','refunded'],'finance'=>['transactions','payments','wallets','withdrawals','settlements','invoices','commissions','taxes'],'subscriptions'=>['subscriptions','plans','renewals','changes'],'assets'=>['library','folders','mine','purchased','storage'],'reader'=>['library','bookmarks','highlights','notes','history'],'loyalty'=>['customers','points','tiers','rewards'],'content'=>['pages','blog','categories','media','banners','reviews'],'marketing'=>['discounts','campaigns','affiliates','referrals','ads'],'ai'=>['overview','models','providers','evaluation','prompts','content','usage','settings'],'support'=>['overview','tickets','mine','assignments','sla','knowledge'],'notifications'=>['all','send','templates','campaigns'],'reports'=>['sales','finance','users','products','content','ai','support'],'search'=>['settings','analytics','suggestions'],'sellers'=>['manage','requests','performance','commission'],'workflow'=>['flows','approvals','queues'],'tasks'=>['mine','team','reminders'],'security'=>['logins','sessions','events','audit'],'integrations'=>['services','api','keys','webhooks','external'],'settings'=>['general','store','payment','email','sms','notifications','security'],'system'=>['health','events','queue','cache','backup','restore','maintenance','features'],'developer'=>['docs','apps','oauth','webhooks','sandbox'],'newsletter'=>['subscribers','campaigns','offers'],'sms'=>['otp','sent','settings']];
$special=[
'users'=>['همه کاربران'=>route('admin.users.index'),'کاربران فعال'=>route('admin.users.index',['status'=>'active']),'در انتظار تأیید'=>route('admin.users.index',['status'=>'inactive']),'نقش‌ها و دسترسی‌ها'=>route('admin.roles.index')],
...
'integrations'=>['کلیدهای API'=>route('admin.integrations.keys')],
];
@endphp
@foreach($groups as $key=>$group)
@if($user->hasPermission($group[1]))
<div class="panel-nav-group {{ str_starts_with($path,'admin/'.$key)?'open':'' }}"><button type="button" class="panel-nav-parent" aria-expanded="{{ str_starts_with($path,'admin/'.$key)?'true':'false' }}"><x-icon name="{{ $icons[$key]??'folder' }}" size="17"/><span>{{ $group[0] }}</span><x-icon name="arrow-left" size="14"/></button><div class="panel-submenu">@foreach($group[2] as $i=>$sub)@php($href=$special[$key][$sub]??route('admin.module',['module'=>$key,'section'=>$sectionKeys[$key][$i]??'overview']))<a class="{{ request()->url()===$href?'active':'' }}" href="{{ $href }}">{{ $sub }}</a>@endforeach</div></div>
@endif
@endforeach
@endif
</nav>