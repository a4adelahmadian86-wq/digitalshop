...
<div class="account-shell">
    <aside class="account-sidebar" id="accountSidebar">
        <div class="account-brand">
            <span class="account-brand-mark">D</span>
            <div><strong>حساب کاربری</strong><small>پنل خریدار</small></div>
        </div>
        <div class="account-sidebar-user">
            <div class="account-avatar">{{ mb_substr(trim(($user->first_name ?? '').' '.($user->last_name ?? '')) ?: 'ک', 0, 1) }}</div>
            <div><strong>{{ trim(($user->first_name ?? '').' '.($user->last_name ?? '')) ?: 'کاربر' }}</strong><small dir="ltr">{{ $user->phone }}</small></div>
        </div>
    ...
    <main class="account-main">
        <header class="account-header">
            <div class="account-header-right"><button class="account-menu" type="button" aria-label="منو" onclick="document.getElementById('accountSidebar').classList.toggle('open')">☰</button><div class="account-header-title"><span>حساب من</span><strong>{{ trim(($user->first_name ?? '').' '.($user->last_name ?? '')) ?: 'کاربر' }}</strong></div></div>
            <div class="account-header-left"><a class="account-header-icon" href="{{ route('account.notifications') }}" aria-label="اعلان‌ها">◉ @if($user->unreadNotifications()->count())<i></i>@endif</a><div class="account-phone" dir="ltr">{{ $user->phone }}</div></div>
        </header>
        <div class="account-content">
            @if(session('success'))<div class="account-alert success">{{ session('success') }}</div>@endif
            @if(session('error'))<div class="account-alert error">{{ session('error') }}</div>@endif
