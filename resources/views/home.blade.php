    ...
    <div class="container">
        <section class="section category-section">
            ...
            <div class="categories-modern">
                ...
                @empty
                    <div class="empty-state"><strong>دسته‌بندی‌ای ثبت نشده است.</strong></div>
                @endforelse
            </div>
        </section>
                    ...
                    <article class="product-card-modern">
                        <a href="{{ route('product.show', $product) }}" class="product-media">
                            @if(!empty($product->thumbnail))
                                <img src="{{ asset($product->thumbnail) }}" alt="{{ $product->title }}" loading="lazy" decoding="async" onerror="this.style.display='none';this.nextElementSibling.style.display='grid'">
                                <div class="product-placeholder" style="display:none"><span>▤</span><small>فایل دیجیتال</small></div>
                            @else
                        ...
                        <div class="product-card-body">
                            ...
                            <div class="product-meta">
                                ...
                                <form method="POST" action="{{ route('cart.add', $product) }}">@csrf<button type="submit" aria-label="افزودن به سبد">🛒</button></form>
                            </div>
                        </div>
                    </article>
                @empty
        ...
        <section class="promo-banner">
            <div>
                <span class="section-kicker">برای شروع</span>
                <h2>فایل مناسب خودت را پیدا کن</h2>
                <p>در فروشگاه جستجو کن و از بین محصولات منتشرشده انتخاب کن.</p>
            </div>
            <a href="{{ route('products.index') }}">ورود به فروشگاه <span>←</span></a>
        </section>

        <section class="section why-section">
            <div class="section-head-modern centered">
                <div><span class="section-kicker">چرا فایل‌مارکت؟</span><h2>یک تجربه ساده برای خرید فایل</h2></div>
            </div>
            <div class="why-grid">
