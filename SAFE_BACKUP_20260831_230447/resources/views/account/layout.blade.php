    ...
    <aside class="account-sidebar" id="accountSidebar">
        <div class="account-brand">
            ...
            <div>
                ...
                <small>پنل خریدار</small>
            </div>
        </div>

        <div class="account-sidebar-user">
            <div class="account-avatar">
                {{ mb_substr(trim(($user->first_name ?? '').' '.($user->last_name ?? '')) ?: 'ک', 0, 1) }}
            </div>
            <div>
                <strong>{{ trim(($user->first_name ?? '').' '.($user->last_name ?? '')) ?: 'کاربر' }}</strong>
    ...
    <main class="account-main">
        <header class="account-header">
            <div class="account-header-right">
                <button class="account-menu" type="button" aria-label="منو" onclick="document.getElementById('accountSidebar').classList.toggle('open')">☰</button>
            ...
            <div class="account-header-left">
                <a class="account-header-icon" href="{{ route('account.notifications') }}" aria-label="اعلان‌ها">◉ @if($user->unreadNotifications()->count())<i></i>@endif</a>
                <div class="account-phone" dir="ltr">{{ $user->phone }}</div>
            </div>
        </header>
...
<script>
    document.addEventListener('click', function (event) {
        ...
        if (window.innerWidth <= 900 && sidebar && sidebar.classList.contains('open') && !sidebar.contains(event.target) && !menu.contains(event.target)) {
